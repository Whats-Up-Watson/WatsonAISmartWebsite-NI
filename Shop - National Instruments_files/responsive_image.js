...
**************************************************************************/var s7riCallbacks=s7riCallbacks||[];var s7riErrCallbacks=s7riErrCallbacks||[];if(typeof s7viewers=="undefined"){s7viewers={};}
if(typeof s7responsiveImage=="undefined"){function s7RIJSONResponse(inArg,inId){for(var i=0;i<s7riCallbacks.length;i++){if(s7riCallbacks[i]!=null){if(s7riCallbacks[i].id==parseInt(inId)){if(s7riCallbacks[i].callback){s7riCallbacks[i].callback(inArg);}
...
return h;}
function devPixelRatio(){var retVal=1;if(window.devicePixelRatio){retVal=window.devicePixelRatio;}else if(screen.deviceXDPI){retVal=window.screen.deviceXDPI/window.screen.logicalXDPI;}else if("matchMedia"in window&&window.matchMedia){if(window.matchMedia("(min-resolution: 2dppx)").matches||win.matchMedia("(min-resolution: 192dpi)").matches){retVal=2;}else if(window.matchMedia("(min-resolution: 1.5dppx)").matches||window.matchMedia("(min-resolution: 144dpi)").matches){retVal=1.5;}}
return retVal;};function updateZoomFactor(){var orientation="landscape";var zoom=1;if(typeof(window.orientation)!="undefined"){orientation=((screen.width/screen.height)>1)?"landscape":"portrait";zoom=document.documentElement.clientWidth/window.innerWidth;return{zoom:zoom,orient:orientation};}else{if(document.documentElement.clientWidth>document.documentElement.clientHeight){orientation="landscape";}else{orientation="portrait";}
...
var testimg=inImageElem;var arrBp="";var breakpointConfig=$(inImageElem).parentsUntil($("body"),".s7dm-dynamic-media").data("media-sizing");if(!breakpointConfig){arrBp=testimg.getAttribute("data-breakpoints");}else{var browserSize=NI.utils.browserSize();var arrBp=breakpointConfig[browserSize];inImageElem.setAttribute("data-breakpoints",arrBp);}
if(!testimg){return;}
...
var oldW=0;var oldTestImgWidth=0;var newW=-1;var maxBreakpoints=0;var minBreakpoints=0;var maxBreakpointsIsCommands="";var minBreakpointsIsCommands="";var newIsCommands="";var pixelRation=devPixelRatio();var scalemode=testimg.getAttribute("data-scalemode")||"upscale";var breakpoints=(arrBp&&arrBp.split(",").length>0)?arrBp.split(","):null;if(breakpoints){for(var i=0;i<breakpoints.length;i++){var data=breakpoints[i].split(":");if(data.length>1){breakpoints[i]={width:data[0]*pixelRation,isCommands:data[1]};}else{breakpoints[i]={width:data[0]*pixelRation,isCommands:""};}}}
...
var initialization=false;var ua=navigator.userAgent.toLowerCase();var isIE=ua.indexOf('msie');var ver=parseFloat(navigator.appVersion);sjGetResponse('req=props,json&scl=1',dataSrc,function(inArg){initialization=true;testimg.style.width="100%";testimg.maxWidth=parseInt(inArg["image.width"]);testimg.style.width=testimg.offsetWidth>=testimg.maxWidth?testimg.maxWidth+"px":"100%";},function(inArg){if(typeof window.console!="undefined"){window.console.log('failed loading req=set for image ['+inURL+']: '+inArg.message);}});function winResized(){if(!testimg||!testimg.parentNode||testimg.parentNode.nodeType===11){if(tmr){clearInterval(tmr);tmr=null;}
return;}
...
break;}}}
if((newW!=oldW)||(oldTestImgWidth!=testimg.offsetWidth)){var tmpOldW=newW;var tmpOldTestImgWidth=testimg.offsetWidth;var newImage=new Image();newImage.onload=function(){oldW=tmpOldW;oldTestImgWidth=tmpOldTestImgWidth;testimg.src=newImage.src;var ratio=newImage.width/newImage.height;testimg.style.width="100%";if((isIE!=-1)&&(ver<9)){testimg.style.height=testimg.offsetWidth/ratio+"px";}};newImage.onerror=function(){newImage.src=testimg.src;};newImage.onabort=function(){};newW=Math.min(parseInt(testimg.maxWidth),newW);newImage.src=dataSrc+((dataSrc.indexOf("?")==-1)?"?":"&")+((newIsCommands!="")?"&"+newIsCommands:"");}};var tmr=setInterval(winResized,500);};}